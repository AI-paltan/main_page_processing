...
class CashFlowDataBucketing():
    ...
    def fetch_report(self):
        ...
        # app.logger.debug(f'BUCKET DATAFRAME:\n {self.df_nlp_bucket_master}')
        # app.logger.debug('============================================================================================')
        ...
        # app.logger.debug(f'DRILL-DOWN DATAFRAME \n{self.df_drilldown_gen}')
        ...
        rem_cols = ['id', 'statement_section', 'statement_sub_section', 'statement_type', 'target_keyword',
                    ...
                    'note_keyword', 'flag_model_training', 'field_tag', 'remarks', 'custom_formula', 'ref_notes_key']
    ...
    def get_section_subtotal(self):
        ...
        for section_key, list_keywords in subtotal_keywords.items():
            ...
            # app.logger.debug(f'{section_key} | {list_keywords}')
            ...
            for idx in reversed(df_section_datasheet.index):
                res_match = self.obj_techfuzzy.token_sort_pro(self.df_datasheet.loc[idx, 'Particulars'],
                                                              list_keywords)
                # app.logger.debug(f'{self.df_datasheet.loc[idx, "Particulars"]} | {res_match}')
                if res_match[0][1] >= 85:
                    # app.logger.debug(f'{section_key} subtotoal | {self.df_datasheet.loc[idx, "Particulars"]}')
                    for curr_year in self.years_list:
        ...
        # app.logger.debug(f'CCF SECTION SUBTOTAL DETAILS | {self.section_subtotal}')
    ...
    def process_operating_bucket(self, section_key='operating_activities'):
        ...
        # app.logger.debug(f'OPERATING ACTIVITY DATA:\n {df_data}')
        # app.logger.debug(f'OPERATING ACTIVITY BUCKET:\n {df_bucket}')
        ...
        # app.logger.debug(f'NET WORKING CAPITAL Operating Activities: {adj_sum_yearly}')
        ...
        for bucket_index, bucket_row in self.df_response.iterrows():
            if bucket_row['target_keyword']:
                ...
                if res_match[0][1] >= 90:
                    for curr_year in self.years_list:
                        # app.logger.debug(f'{type(curr_year)} | ')
    ...
    def process_investing_bucket(self, section_key='investing_activities'):
        ...
        # app.logger.debug(f'INVESTING ACTIVITY DATA:\n {df_data}')
        # app.logger.debug(f'INVESTING ACTIVITY BUCKET:\n {df_bucket}')
    ...
    def process_financing_bucket(self, section_key='financing_activities'):
        ...
        # app.logger.debug(f'FINANCING ACTIVITY DATA:\n {df_data}')
        # app.logger.debug(f'FINANCING ACTIVITY BUCKET:\n {df_bucket}')
    ...
    def dynamic_bucketing(self, df_data, df_bucket, section_key):
        ...
        for curr_year in self.years_list:
            # app.logger.debug(f'FINANCIAL_YEAR : {curr_year}')
            ...
            for bucket_index, bucket_row in df_bucket.iterrows():
                app.logger.debug(f'Bucket Keyword : {bucket_row[str("primary_keywords")]}')

                if bucket_row['field_tag'] == 'balancing_value':
                    index_balancing_column = bucket_index
                    continue
                elif bucket_row['field_tag'] == 'subtotal':
                ...
                # app.logger.debug(f'BEST MATCH: {best_match}')
                if len(best_match['data_index']) > 0:
                    ...
                    # app.logger.debug(f'MATCH FOUND {best_match}')
                else:
                    pass
                    # app.logger.debug(f'NO MATCH FOUND')

                # app.logger.debug('-----------')

            # CALCULATE BALANCING FIGURE MANUALLY - FOR THE YEAR
            # app.logger.debug(f'BALANCING INDEX FOUND AT: {index_balancing_column}')
            ...
            # app.logger.debug(f'SUBTOTAL INDEX FOUND AT: {index_subtotal_column}')
        ...
        # app.logger.debug('=========================================================')
        # app.logger.debug(f'NET SUBTOTAL : {net_bucket_total}')
        # app.logger.debug(f'----- AFTER PROCESSING BUCKET DATA ----- \n{df_bucket}')
    ...
    def direct_datafetch(self, curr_year, bucket_row, df_data, fetch_type='direct'):
        ...
        for data_index, data_row in df_data.iterrows():
            ...
            if res_fuzz_match[0][1] >= self.conf_score_thresh:
                if bucket_row['field_tag'] == 'positive' and float(data_row[str(curr_year)]) < 0:
                    continue
                elif bucket_row['field_tag'] == 'negative' and float(data_row[str(curr_year)]) > 0:
                    continue
